...
def explanation(row, budget, max_flight, weights):
    ...
    if row["estimated_trip_cost"] > budget:
        tradeoffs.append("the estimated trip cost is above your budget")

    if row["flight_hours"] > max_flight:
        tradeoffs.append("the travel time exceeds your preferred maximum")

    if row["rain_days"] >= 4:
        tradeoffs.append("the season can be relatively rainy")

    if row["beach_score"] < 50 and weights["Beach"] >= 7:
        tradeoffs.append("it is not a particularly strong beach destination")

    if row["nightlife_score"] < 50 and weights["Nightlife"] >= 7:
        tradeoffs.append("the nightlife is weaker than you prefer")
    ...
    return (
        ...
        f"and travel time is about {row['flight_hours']:.1f} hours "
        f"from Philadelphia. "
...
st.sidebar.title("Plan Your Trip ✈️")
...
fmin = scored["sample_roundtrip_flight"].min()
fmax = scored["sample_roundtrip_flight"].max()

tmin = scored["flight_hours"].min()
tmax = scored["flight_hours"].max()

scored["flight_cost_score"] = scored[
    "sample_roundtrip_flight"
].apply(
    lambda x: inverse_score(x, fmin, fmax)
)

scored["flight_time_score"] = scored[
    "flight_hours"
].apply(
    lambda x: inverse_score(x, tmin, tmax)
)

scored["weather_match_score"] = scored.apply(
    lambda r: weather_score(
        r["avg_temp_f"],
        r["rain_days"],
        preferred_low,
        preferred_high
...
scored["budget_match_score"] = scored[
    "estimated_trip_cost"
].apply(
    lambda x: budget_score(x, budget)
)

scored["interest_match_score"] = scored.apply(
    lambda r: interest_score(r, weights),
    axis=1
)
...
st.title("Where Should You Go Next? ✈️")

st.markdown(
    "Personalized destination recommendations based on your budget, "
    "travel style, weather preferences, and interests."
)
...
">
    ...
    <p style="margin:0; font-size:17px;">
        Compare destinations using cost, weather, travel time,
...
st.subheader(
    f"Your Best Match: {top['city']}, {top['country']} 🌍"
)
...
c.metric(
    "Sample airfare",
    f"${top['sample_roundtrip_flight']:,.0f}"
)

d.metric(
    "Travel time",
    f"{top['flight_hours']:.1f} hrs"
)
...
st.markdown("### Other Destinations You'll Love")
...
st.markdown("### Top 10 Destination Ranking")

st.bar_chart(
    scored
    .head(10)
    .set_index("city")[["overall_score"]]
)
...
with st.expander("How the model works"):
    ...
    st.markdown("""
**Inputs:**  
Budget, trip length, maximum preferred travel time,
weather preferences, and traveler interests.

**Normalization:**  
Airfare, travel time, budget fit, weather, and destination
characteristics are converted into comparable 0–100 scores.
...
**Output:**  
Destinations are ranked using a personalized multi-criteria
decision score. The recommendation explains both strengths
and trade-offs.

**Future version:**  
The model can connect to live airfare, weather, attraction,
and generative-AI APIs.
""")


st.caption(
    "MBA AI project prototype. Airfare and destination values "
    "currently use demo data rather than live quotes."
)
